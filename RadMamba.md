...
#### 4. Mamba 状态空间骨干网络 (The SSM Backbone)
经过前三步，原本大而笨重的雷达图，已经被压成了一根又细又长、且富含物理意义的面条（1D Sequence）。这时候，Mamba 登场了。它通过前面提到的**“选择性扫描（Selective Scan）”**，像一个极其聪明的筛子，把面条里代表“走路”、“跌倒”的黄金片段记住，把“静止不动”的废料扔掉。最后送入 Linear 层输出概率。
