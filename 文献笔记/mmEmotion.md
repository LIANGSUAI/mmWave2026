...
论文使用 SAM（Self-Assessment Manikin，自我评估小人）量表标注情感状态，而不是直接使用“开心”“悲伤”这类离散标签。SAM 将情绪拆成三个连续维度，每个维度打 1-9 分：

- **Valence（效价）**：愉悦程度。`1` 表示非常不愉快/悲伤，`9` 表示非常愉快/开心。
- **Arousal（唤醒度）**：兴奋程度。`1` 表示非常平静/困倦，`9` 表示非常紧张/兴奋。
- **Dominance（支配度）**：控制感。`1` 表示非常被动/无力，`9` 表示非常主动/有控制力。
...
- 所有指标差异均在 ±3% 以内，说明复现结果与论文 Table 7 基本一致。
- PPG/GSR 复现结果整体略优于论文，可能来自随机种子、划分细节或网格搜索差异。
- mmWave Valence 略低，Accuracy 差异为 -2.5%，可能与 EMD 分解使用带通滤波近似有关。
- SVM 网格搜索和交叉验证划分存在一定随机性，是复现差异的重要来源。
- mmWave 在 Arousal 和 Dominance 上接近 PPG/GSR，但 Valence 略低，说明非接触雷达生命体征对部分情绪维度有效，但信息量仍受限于心跳/呼吸特征。
