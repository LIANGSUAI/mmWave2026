...
作者把它称为 **CP-Mamba Block**。切好的多普勒切片，在送入真正的 Mamba 之前，先用一个一维卷积滑动扫描一下。这不仅起到了降维的作用，还能让切片和切片之间产生“相邻的连贯性”。实际上是将线性投影层换成了一维卷积

#### 4. Mamba 状态空间骨干网络 (The SSM state-space model（状态空间模型） Backbone)
经过前三步，原本大而笨重的雷达图，已经被压成了一根又细又长、且富含物理意义的（1D Sequence）。这时候，Mamba 它通过前面提到的“选择性扫描（Selective Scan）”，把1D Sequence里代表“走路”、“跌倒”的黄金片段记住，把“静止不动”的废料扔掉。最后送入 Linear 层输出概率。
