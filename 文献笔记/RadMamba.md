...
- **极致的参数效率**：在 DIAT 数据集上，RadMamba 达到了 99.8% 的准确率，仅需 21.7k 个参数，这只有近期基于 SSM 模型的 1/400 。
- **算力需求极低**：在动作更多的 CI4R 数据集上，取得了 91.2% 的准确率，但每样本推理的浮点运算数 (#FLOP/Inf.) 仅为 8.8M，远低于之前 Transformer 模型的 169M 至 2410M 。
- **连续动作的统治力**：在最复杂的连续序列 UoG20 上，RadMamba 以极小的 6.7k 参数量和 1.0M #FLOP/Inf. 实现了 89.3% 的准确率，击败了参数量大得多的基线模型 。
### 数据集
