...
- 半监督学习部分适合借鉴到毫米波雷达场景：真实跌倒、异常动作或少样本动作都存在标注困难，大量无标注日常活动数据更容易获得。
- 可以重点关注“自适应置信度/样本难度”的思想：高置信样本用于扩充训练集，边界样本保留给人工核验或后续精标。
- 时空融合部分适合和 [[Star Graph + DDGNN]]、[[RadMamba]] 对比：一个强调空间-时间特征融合，一个强调稀疏点云图结构，一个强调长序列高效建模。
